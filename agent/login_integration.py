# agent/login_integration.py
"""
Integración de LoginAction con ActionExecutor
ARCHIVO PARA AGREGAR AL action_executor.py
"""

# ============================================
# IMPORTACIONES ADICIONALES PARA action_executor.py
# ============================================
# Agregar al inicio de action_executor.py:
from login_action import LoginAction

# ============================================
# MODIFICAR __init__ de ActionExecutor
# ============================================
# En ActionExecutor.__init__, agregar:
...
# ============================================
# AGREGAR CASO EN _execute_action_internal
# ============================================
# En el método _execute_action_internal, después del elif action_type == "login":
...
# ============================================
# AGREGAR MÉTODO _advanced_login
# ============================================
# Agregar al final de la clase ActionExecutor:

    async def _advanced_login(self, driver: webdriver.Chrome, params: Dict) -> bool:
        """
        Login avanzado con detección de problemas
        
        Params:
        {
            "site": "ecuabet" | "melbet" | "generic",
            "username": "user@example.com",
            "password": "password123",
            "username_selector": "...",  # Opcional
            "password_selector": "...",  # Opcional
            "submit_selector": "...",  # Opcional
            "expected_success_url": "...",  # Opcional
            "wait_after_submit": 5
        }
        """
        
        try:
            # Ejecutar login con detección
            result = await self.login_action.execute_login(driver, params)
            
            # Log resultado
            if result["success"]:
                logger.info(f"✅ Login successful on {params.get('site', 'custom')}")
                
                if result.get("details"):
                    logger.debug(f"Success details: {result['details']}")
                
                return True
            
            else:
                issue = result.get("issue_detected", "unknown")
                logger.error(f"❌ Login failed on {params.get('site', 'custom')}: {issue}")
                
                if result.get("details"):
                    logger.error(f"Failure details: {result['details']}")
                
                if result.get("screenshot"):
                    logger.info(f"Screenshot saved: {result['screenshot']}")
                
                # Si es reCAPTCHA, esperar más tiempo (intervención manual posible)
                if issue == "recaptcha":
                    logger.warning("⚠️ reCAPTCHA detected - manual intervention may be required")
                    await asyncio.sleep(30)  # Esperar 30s por si alguien lo resuelve
                
                return False
        
        except Exception as e:
            logger.error(f"Advanced login execution error: {e}")
            return False
