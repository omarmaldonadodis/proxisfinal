# adspower-orchestrator2/app/api/v1/warming.py (ACTUALIZACIÓN COMPLETA)
...
from app.services.warming_batch_executor import WarmingBatchExecutor
...
async def execute_batch_warming(
    ...
    """
    ✅ EJECUCIÓN PARALELA SINCRONIZADA
    
    Todos los navegadores ejecutan las mismas acciones AL MISMO TIEMPO
    - Se eliminan parámetros innecesarios (max_parallel, computer_ids)
    - El sistema asigna automáticamente según disponibilidad
    - Ejecuciones múltiples no se bloquean entre sí
    ...
    try:
        executor = WarmingBatchExecutor(db)
...
async def get_batch_status(
    ...
    """Obtiene estado de un batch en ejecución"""
        ...
        while True:
            ...
            if message_type == "heartbeat":
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    # ✅ Notificar al batch executor
                    from app.services.warming_batch_executor import WarmingBatchExecutor
                    executor = WarmingBatchExecutor(db)
                    executor.mark_execution_completed(execution_id, profile_id)
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    if not recovery_result.get("recovered"):
                        ...
                        # ✅ Notificar al batch executor
                        executor = WarmingBatchExecutor(db)
                        executor.mark_execution_failed(execution_id, profile_id)
