...
async def list_computers_with_metrics(db: AsyncSession = Depends(get_db)):
    ...
    for computer, hc in rows:
        ...
        # BUG 2 FIX: si la última hc tiene ram=0 (health-check de AdsPower),
        # buscar la más reciente con datos reales del agente
        ...
        if ram_value == 0:
            real_hc_result = await db.execute(
                select(HealthCheck)
                .where(
                    HealthCheck.computer_id == computer.id,
...
async def get_computer_metrics(
    ...
    BUG 2 FIX: filtra entradas con memory_usage = 0 — son del health-check
    de AdsPower que no contiene datos del sistema operativo.
    """
    ...
    result = await db.execute(
        ...
        .where(
            ...
            HealthCheck.memory_usage > 0,   # ← BUG 2 FIX
        )
...
async def get_computer_metrics_latest(
    ...
    """Métrica más reciente con datos reales del sistema"""
    result = await db.execute(
        select(HealthCheck)
        .where(
            HealthCheck.computer_id == computer_id,
            HealthCheck.memory_usage > 0,   # ← solo métricas reales
        )
        .order_by(desc(HealthCheck.checked_at))
        .limit(1)
    )
    ...
    for s in sessions:
        if s.status == SessionStatus.ACTIVE:
            ...
            logs.append({
                "timestamp": s.closed_at.isoformat() if s.closed_at else s.requested_at.isoformat(),
                "level": "ERROR",
                "message": f"Sesión CRASHEADA — {s.error_detail or 'unknown error'} [{s.agent_name}]"
            })

    # Agregar métricas de health check (solo entradas con datos reales del sistema)
    for h in reversed(checks):
        if not h.memory_usage or h.memory_usage == 0:
            continue
        mem = f"{h.memory_usage}%" if h.memory_usage is not None else "N/A"
        logs.append({
            "timestamp": h.checked_at.isoformat(),
            "level": "INFO" if h.is_healthy else "WARNING",
            "message": f"cpu={h.cpu_usage}% mem={mem} disk={h.disk_usage}% adspower={h.adspower_status}"
        })

    # Combinar con logs en memoria del connection_manager (real-time)
...
def _compute_uptime(last_seen: datetime) -> str:
    """
    BUG 3 FIX:
    - Evita resultado negativo por mismatch de timezone
    - Muestra minutos cuando el uptime es menor a 1 hora (antes devolvía "0h")
    - Devuelve "—" si el nodo nunca se ha visto
    """
    if not last_seen:
        return "—"

    now  = datetime.utcnow()
    last = last_seen.replace(tzinfo=None)

    # Guard: timezone mismatch puede hacer que last > now
    if last > now:
        return "0m"
